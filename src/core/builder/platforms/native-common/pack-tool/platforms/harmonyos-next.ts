...
export default class HarmonyOSNextPackTool extends NativePackTool {
    declare params: CocosParams<OHOSParam>;
    ...
    async create() {
        ...
        const moduleJSON = this.readJSON5Sync(moduleFile);
        ...
        const configJSON = this.readJSON5Sync(cfgFile);
    ...
    async run(): Promise<boolean> {
        ...
        const configJson = this.readJSON5Sync(ps.join(projectDir, 'entry/src/main/module.json5'));
