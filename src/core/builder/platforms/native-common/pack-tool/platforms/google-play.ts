...
export interface IGooglePlayParams {
    packageName: string;
    sdkPath: string;
    ndkPath: string;
    javaHome: string;
...
export default class GooglePlayPackTool extends NativePackTool {
    declare params: CocosParams<IGooglePlayParams>;
    ...
    protected async updateAndroidGradleValues() {
        ...
        if (fs.existsSync(gradlePropertyPath)) {
            ...
            content = content.replace(/COCOS_ENGINE_PATH=.*/, `COCOS_ENGINE_PATH=${cchelper.fixPath(this.params.nativeEnginePath)}`);
