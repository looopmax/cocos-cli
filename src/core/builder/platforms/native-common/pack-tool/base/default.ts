...
export default abstract class NativePackTool {
    ...
    private validateTemplateVersion(): boolean {
        ...
        const semverEngineVersion = engineVersion && semver.valid(engineVersion);
        const semverProjectVersion = projEngineVersion && semver.valid(projEngineVersion);
        const semverVersionRange = semver.validRange(versionRange);
        if (semverEngineVersion && semverProjectVersion && semverVersionRange) {
            if (semver.satisfies(semverProjectVersion, semverVersionRange, { includePrerelease: true })) {
                if (semver.gt(semverProjectVersion, semverEngineVersion)) {
